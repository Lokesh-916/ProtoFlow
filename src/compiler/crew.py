...
async def run_pipeline(session: PipelineSession) -> None:
    ...
    session.intent = await _run_stage(
        ...
        "groq/openai/gpt-oss-20b", _stage_intent()
    )
    ...
    session.architecture = await _run_stage(
        ...
        "groq/openai/gpt-oss-20b", _stage_architecture()
    )
    ...
    db_result = await _run_schema_stage("db_schema", _stage_db, "groq/openai/gpt-oss-20b")
    api_result = await _run_schema_stage("api_schema", _stage_api, "groq/openai/gpt-oss-20b")
    ui_result = await _run_schema_stage("ui_schema", _stage_ui, "groq/openai/gpt-oss-20b")
    auth_result = await _run_schema_stage("auth_schema", _stage_auth, "groq/openai/gpt-oss-20b")
    ...
    for attempt in range(1, MAX_REPAIR_LOOPS + 1):
        ...
        validation = await _run_stage(
            session, "validation",
            "groq/openai/gpt-oss-20b", _stage_validate()
        )
        ...
        await _run_stage(
            session, "repair",
            "groq/openai/gpt-oss-20b", _stage_repair()
        )
    ...
    await _run_stage(
        ...
        "groq/openai/gpt-oss-20b", _stage_runtime()
    )
    ...
    await _run_stage(
        session, "logging",
        "groq/openai/gpt-oss-20b", _stage_logging()
    )
